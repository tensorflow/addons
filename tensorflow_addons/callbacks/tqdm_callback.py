...
class TQDMCallback(Callback):
    ...
    Arguments:
        ...
        output_file: output file (default sys.stderr)
    """
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if self.show_inner:
            self.tqdm_inner = tqdm(
                desc=desc, total=self.inner_total, leave=self.leave_inner)
        self.inner_count = 0
        self.running_logs = {}
    ...
    def on_train_begin(self, logs={}):
        if self.show_outer:
            ...
            self.tqdm_outer = tqdm(
                desc=self.outer_description, total=epochs, leave=self.leave_outer)
