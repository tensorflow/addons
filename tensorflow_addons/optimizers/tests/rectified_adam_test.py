...
def run_sparse_sample(iterations, expected, optimizer):
    ...
    grad_0 = tf.IndexedSlices(tf.constant(
        [0.1]), tf.constant([0]), tf.constant([2]))
    grad_1 = tf.IndexedSlices(tf.constant(
        [0.04]), tf.constant([1]), tf.constant([2]))
...
def test_schedulers():
    ...
    run_dense_sample(
        ...
        optimizer=RectifiedAdam(
            learning_rate=lr_scheduler, weight_decay=wd_scheduler),
    )
