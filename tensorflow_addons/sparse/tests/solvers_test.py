...
def test_bicgstab_solver(batching, adjoint_a):
    ...
    csr_mat = tf.raw_ops.DenseToCSRSparseMatrix(dense_input=mat, indices=tf.where(mat))
    tf.debugging.assert_near(
        bicgstab_solver(a=csr_mat, b=rhs, adjoint_a=adjoint_a),
        expected_solution,
        rtol=1e-10,
    )
...
def test_bicgstab_solver_gradient(batching, adjoint_a):
    ...
    with tf.GradientTape() as tape:
        csr_mat = tf.raw_ops.DenseToCSRSparseMatrix(
            dense_input=mat, indices=tf.where(mat)
        )
