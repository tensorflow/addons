...
class RreluBenchmark(tf.test.Benchmark):
    def benchmarkRreluOp(self):
        ...
        lower = 0.1
        upper = 0.2
        with tf.compat.v1.Graph().as_default(),tf.compat.v1.Session(config=tf.test.benchmark_config()) as sess:
            result = rrelu(x, lower, upper, training=True)
            self.run_op_benchmark(sess,result,min_iters=25,name="rrelu_test")
