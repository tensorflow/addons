...
def sparsemax_loss(logits, sparsemax, labels, name=None):
  ...
  function. The implementation is a reformulation of the original loss function
  such that it uses the sparsemax properbility output instead of the internal
  \tau variable. However, the output is identical to the original loss function.
