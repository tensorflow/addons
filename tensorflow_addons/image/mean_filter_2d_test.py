...
import mean_filter_2d as md
from tensorflow_addons.utils import test_utils


@test_utils.run_all_in_graph_and_eager_modes
class Mean2DTest(tf.test.TestCase):
    def _validateMean_2d(self, inputs, expected_values, filter_shape=(3, 3)):

        values_op = md.mean_filter_2D(inputs)
    ...
    def testfiltertuple(self):
        ...
        with self.assertRaisesRegexp(TypeError,
            ...
            md.mean_filter_2D(tf_img, 3)
            md.mean_filter_2D(tf_img, 3.5)
            md.mean_filter_2D(tf_img, 'dt')
            md.mean_filter_2D(tf_img, None)

        filter_shape = (3, 3, 3)
        msg = 'Filter shape must be a tuple of 2 integers. ' \
              'Got %s values in tuple' % len(filter_shape)
        with self.assertRaisesRegexp(ValueError, msg):
            md.mean_filter_2D(tf_img, filter_shape)

        with self.assertRaisesRegexp(TypeError,
                                     'Size of the filter must be Integers'):
            md.mean_filter_2D(tf_img, (3.5, 3))
            md.mean_filter_2D(tf_img, (None, 3))

    def testfiltervalue(self):
        tf_img = tf.zeros([3, 4, 3], tf.int32)

        with self.assertRaises(ValueError):
            md.mean_filter_2D(tf_img, (4, 3))
    ...
    def testDimension(self):
        ...
        with self.assertRaises(TypeError):
            md.mean_filter_2D(tf_img)
            md.mean_filter_2D(tf_img1)
            md.mean_filter_2D(tf_img2)
