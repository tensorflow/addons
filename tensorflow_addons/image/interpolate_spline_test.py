...
class _InterpolationProblem(object):
    ...
    def get_problem(self, optimizable=False, extrapolate=True,
        ...
        train_points = (tf.Variable(init_points)
                        if optimizable else tf.constant(init_points))
...
class _QuadraticPlusSinProblem1D(_InterpolationProblem):
    ...
    HARDCODED_QUERY_VALUES = {
        (1.0, 0.0):
        [6.2647187603, -7.84362604077, -5.63690142322, 1.42928896387],
        (1.0, 0.01):
        [6.77688289946, -8.02163669853, -5.79491157027, 1.4063285693],
        (2.0, 0.0):
        [8.67110264937, -8.41281390883, -5.80190044693, 1.50155606059],
        (2.0, 0.01): [
            6.70797816797, -7.49709587663, -5.28965776238, 1.52284731741
        ],
        (3.0, 0.0): [
            9.37691802935, -8.50390141515, -5.80786417426, 1.63467762122
        ],
        (3.0, 0.01): [
            4.47106304758, -5.71266128361, -3.92529303296, 1.86755293857
        ],
        (4.0, 0.0): [
            9.58172461111, -8.51432104771, -5.80967675388, 1.63361164256
        ],
        (4.0, 0.01): [
    ...
    def np_function(self, x):
        """Takes np array, evaluates the test function, and returns np
        array."""
        return np.sum(
    ...
    def tf_function(self, x):
        """Takes tf tensor, evaluates the test function,  and returns tf
        tensor."""
        return tf.reduce_mean(
            tf.pow((x - 0.5), 3) - 0.25 * x + 10 * tf.sin(x * 10),
            2,
            keepdims=True)
...
class _QuadraticPlusSinProblemND(_InterpolationProblem):
    ...
    HARDCODED_QUERY_VALUES = {
        (1.0, 0.0):
        [1.06609663962, 1.28894849357, 1.10882405595, 1.63966936885],
        (1.0, 0.01):
        [1.03123780748, 1.2952930985, 1.10366822954, 1.65265118569],
        (2.0, 0.0):
        [0.627787735064, 1.43802857251, 1.00194632358, 1.91667538215],
        (2.0, 0.01): [
            0.730159985046, 1.41702471595, 1.0065827217, 1.85758519312
        ],
        (3.0, 0.0): [
            0.350460417862, 1.67223539464, 1.00475331246, 2.31580322491
        ],
        (3.0, 0.01): [
            0.624557250556, 1.63138876667, 0.976588193162, 2.12511237866
        ],
        (4.0, 0.0): [
            0.898129669986, 1.24434133638, -0.938056116931, 1.59910338833
        ],
        (4.0, 0.01): [
            0.0930360338179, -3.38791305538, -1.00969032567, 0.745535080382
        ],
    }
    ...
    def tf_function(self, x):
        ...
        return tf.reduce_sum(
            tf.square(x - 0.5) + 0.25 * x + 1 * tf.sin(x * 15),
            2,
            keepdims=True)
