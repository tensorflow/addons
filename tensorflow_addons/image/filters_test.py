...
from tensorflow_addons.image import median_filter1d
...
class MedianFilter1dTest(tf.test.TestCase):
    def _validate_median_filter1d(self,
                                  inputs,
                                  expected_values,
                                  filter_shape=3):
        output = median_filter1d(inputs, filter_shape)
        self.assertAllClose(output, expected_values)
    ...
    def test_filter_integer(self):
        ...
        for filter_shape in [(3, 3), 3.5, 'dt', None]:
            with self.assertRaisesRegexp(TypeError,
                                         'Filter shape must be an Integer'):
                median_filter1d(tf_img, filter_shape)
    ...
    def test_three_channels(self):
        ...
        tf_img = tf.convert_to_tensor(value=tf_img)
        expt = [[[83, 22, 141], [83, 141, 201], [90, 190, 141]],
                [[29, 141, 248], [4, 190, 143], [4, 71, 165]],
                [[93, 211, 143], [143, 230, 49], [93, 230, 6]]]
        expt = tf.convert_to_tensor(value=expt)
        self._validate_median_filter1d(tf_img, expt)
