...
def test_unknown_shape():
  query_points = tf.constant([[0.0, 0.0], [0.0, 1.0], [0.5, 2.0], [1.5, 1.5]],
                             shape=[1, 4, 2])
...
def _check_zero_flow_correctness(shape, image_type, flow_type):
  ...
  rand_image, rand_flows = _get_random_image_and_flows(shape, image_type,
                                                       flow_type)
  rand_flows *= 0
...
def _check_interpolation_correctness(shape,
                                     image_type,
                                     flow_type,
