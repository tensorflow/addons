...
def _assertion_list_for_inputs(grid, query_points):
  """Creates a list of assertions to validate `interpolate_bilinear` inputs."""
  assertion_list = []

  assertion_list.append(tf.assert_rank(grid, 4, "Grid must be 4D Tensor"))

  assertion_list.append(
      tf.Assert(
          tf.greater_equal(tf.shape(grid)[1], 2),
          ["Grid height must be at least 2."]))

  assertion_list.append(
      tf.Assert(
          tf.greater_equal(tf.shape(grid)[2], 2),
          ["Grid width must be at least 2."]))

  assertion_list.append(
      tf.assert_rank(query_points, 3, "Query points must be 3 dimensional."))

  assertion_list.append(
      tf.assert_equal(
          tf.shape(query_points)[2], 2,
...
def interpolate_bilinear(
  ...
  with tf.name_scope(name or "interpolate_bilinear"):
    ...
    with tf.control_dependencies(
        _assertion_list_for_inputs(grid, query_points)):
...
@tf.function
def dense_image_warp(image: types.TensorLike,
                     flow: types.TensorLike,
  ...
  """Image warping using per-pixel flow vectors.
    ...
    Args:
      ...
      name: A name for the operation (optional).  Note that image and flow can
        be of type `tf.half`, `tf.float32`, or `tf.float64`, and do not
...
def dense_image_warp_annotated(image: types.TensorLike,
                               flow: types.TensorLike,
                               name: Optional[str] = None) -> tf.Tensor:
  """Similar to dense_image_warp but annotated with experimental_implements.

    IMPORTANT: This is a temporary function and will be removed after
    TensorFlow's
    next release.
