...
def _iterative_mergesort(y: TensorLike,
  ...
  Returns:
    A tuple consisting of a int32 scalar that counts the number of exchanges
    required to produce a sorted permutation, and a tf.int32 Tensor that
    contains the ordering of y values that are sorted.
  """
  exchanges = 0
  num = tf.size(y)
    ...
    for left in tf.range(0, num - k, 2 * k, dtype=tf.int32):
      ...
      while tf.less(i, rght) and tf.less(j, rend):
        ...
        if tf.less_equal(yij[0], yij[1]):
          ...
          # Explanation here https://www.geeksforgeeks.org/counting-inversions/.
          exchanges += rght - i
          j += 1
        m += 1
...
def kendalls_tau(y_true: TensorLike, y_pred: TensorLike) -> tf.Tensor:
  ...
  Returns:
    Kendell's Tau, the 1945 tau-b formulation that ignores ordering of ties,
    as a scalar Tensor.
  """
...
class KendallsTau(MeanMetricWrapper):
  """Computes how well a model orders items, computing mean-tau acros batches.

  Any types supported by tf.math.less may be used for y_pred and y_true values,
  and these types do not need to be the same as they are never compared against
  each other. The return type of this metric is always tf.float32 and a value
  between -1.0 and 1.0.
  ...
  Note that there is a streaming implementation of an approximate algorithm, but
  this is different from the one implemented here, see:
