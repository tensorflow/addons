...
# ==============================================================================
"""Tests for F1 macro and weighted metric."""
...
from tensorflow_addons.metrics import F1MacroAndWeighted
from tensorflow_addons.utils import test_utils


@test_utils.run_all_in_graph_and_eager_modes
class F1MacroAndWeightedTest(tf.test.TestCase):
    def test_config(self):
        f1_obj = F1MacroAndWeighted(name='f1_macro_and_weighted_score',
                                    num_classes=3)
        self.assertEqual(f1_obj.name, 'f1_macro_and_weighted_score')
        self.assertEqual(f1_obj.dtype, tf.float32)
        self.assertEqual(f1_obj.num_classes, 3)
        # Check save and restore config
        f1_obj2 = F1MacroAndWeighted.from_config(f1_obj.get_config())
        self.assertEqual(f1_obj2.name, 'f1_macro_and_weighted_score')
        self.assertEqual(f1_obj2.dtype, tf.float32)
        self.assertEqual(f1_obj2.num_classes, 3)

    def initialize_vars(self):
        f1_obj = F1MacroAndWeighted(num_classes=3, average='macro')
        ...
        return f1_obj, f1_obj1

    def update_obj_states(self, f1_obj, f1_obj1, actuals, preds):
    ...
    def test_f1_perfect_score(self):
        ...
        # Initialize
        f1_obj, f1_obj1 = self.initialize_vars()
        # Update
        self.update_obj_states(f1_obj, f1_obj1, actuals, preds)
        # Check results
    ...
    def test_f1_random_score(self):
        ...
        self.update_obj_states(f1_obj, f1_obj1, actuals, preds)
        # Check results
        self.check_results(f1_obj, 0.6555555)
        self.check_results(f1_obj1, 0.6777777)
