...
if returncode == 1:
  reply = get_input("Package {} will be installed. Are You Sure? [y/n] ".format(_REQUIRED_PKG))
  if reply in ('y', 'Y'):
    print("> Installing...")
    subprocess.check_call([_PYTHON_PATH, "-m", "pip", "install", _PIP_INSTALL_OPTS, "-r", "requirements.txt"])
  else:
    print("> Exiting...")
    sys.exit()
else:
...
import logging; logging.disable(logging.WARNING); import tensorflow as tf
...
if _TF_NEED_CUDA == 1:
  ...
  INPUT = get_input("Please specify the CUDA version [Default is {}]: ".format(_DEFAULT_CUDA_VERISON))
  _TF_CUDA_VERSION=INPUT if INPUT else _DEFAULT_CUDA_VERISON
  print("> Using CUDA version: {}".format(_TF_CUDA_VERSION))
  print()

  INPUT = get_input("Please specify the location of CUDA. [Default is {}]: ".format(_DEFAULT_CUDA_PATH))
  _CUDA_TOOLKIT_PATH=INPUT if INPUT else _DEFAULT_CUDA_PATH
  print("> CUDA installation path: {}".format(_CUDA_TOOLKIT_PATH))
  print()

  INPUT = get_input("Please specify the cuDNN major version [Default is {}]: ".format(_DEFAULT_CUDNN_VERSION))
  _TF_CUDNN_VERSION=INPUT if INPUT else _DEFAULT_CUDNN_VERSION
  print("> Using cuDNN version: {}".format(_TF_CUDNN_VERSION))
  print()

  INPUT = get_input("Please specify the location of cuDNN installation. [Default is {}]: ".format(_DEFAULT_CUDNN_PATH))
