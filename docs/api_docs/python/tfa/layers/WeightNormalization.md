...
* <b>`data_init`</b>: If `True` use data dependent variable initialization

#### Raises:

* <b>`ValueError`</b>: If not initialized with a `Layer` instance.
* <b>`ValueError`</b>: If `Layer` does not contain a `kernel` of weights
* <b>`NotImplementedError`</b>: If `data_init` is True and running graph execution
